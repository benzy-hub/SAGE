...
export default function AdvisorAcademicPlanViewerPage() {
    ...
    const load = async () => {
      ...
      try {
        const response = await fetch(
          `/api/advisor/students/${encodeURIComponent(studentId)}/academic-plan`,
          { cache: "no-store" },
        );
        ...
        const parsed = payload as PlanPayload;
        setData(parsed);
        setGuidanceDrafts(
          Object.fromEntries(
            parsed.items.map((item) => [item.id, item.advisorGuidance ?? ""]),
          ),
        );
      } catch (err) {
        setError(
  ...
  const saveGuidance = async (itemId: string) => {
    if (!studentId) return;
    setSaving(true);
    setError("");
    try {
      const response = await fetch(
        `/api/advisor/students/${encodeURIComponent(studentId)}/academic-plan/${encodeURIComponent(itemId)}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            advisorGuidance: guidanceDrafts[itemId] ?? "",
          }),
        },
      );
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload?.error ?? "Failed to save guidance");
      }
      toast.success("Advisor guidance saved");
      setData((prev) =>
        prev
          ? {
              ...prev,
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to save guidance",
      );
        ...
        <div>
          <div className="sage-section-chip inline-flex">
            <span className="text-xl sm:text-2xl font-medium text-primary-foreground">
              Student Academic Plan View
            </span>
          </div>
          <p className="mt-3 text-sm text-muted-foreground max-w-3xl">
            Advisor visibility into student roadmap, custom milestones, and
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 space-y-2">
                      <textarea
                        ...
                        rows={2}
                        placeholder="Add advisor guidance for this plan item"
                        className="w-full rounded-lg border border-foreground/20 bg-background px-3 py-2 text-xs resize-none"
                      />
                      <button
                        type="button"
                        disabled={saving}
                        onClick={() => void saveGuidance(item.id)}
